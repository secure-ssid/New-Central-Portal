...
- **Lab** — a sandbox of self-contained tools built on live data: **Device Deep-Dive** (CPU/memory/throughput trends plus switch temperature, PoE budget, VLANs and interface errors), **Compliance Board** (firmware drift, config sync, Central insights), **Activity & History** (device up/down timeline, portal audit trail, client onboarding, cleared-alert post-mortems), **Application Visibility** (DPI application traffic and risk classification with per-client drilldown), network chatbot with RAG (centralmcp LanceDB index) and MCP tool calling, semantic doc search, MCP tool tester, AI health report, running-config viewer (secrets masked), ping tester, alert dashboard, client fingerprints, and a GreenLake Platform explorer.
